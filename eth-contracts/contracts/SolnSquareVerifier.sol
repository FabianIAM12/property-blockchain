...
contract SolnSquareVerifier is RealEstateToken, Verifier {
    ...
    function addSolution(uint[2] memory a, uint[2][2] memory b, uint[2] memory c, uint[2] memory input) public returns (bool)
    {
    ...
    function mintVerified(uint[2] memory a, uint[2] memory a_p, uint[2][2] memory b, uint[2] memory b_p, uint[2] memory c, uint[2] memory c_p, uint[2] memory h, uint[2] memory k, uint[2] memory input, uint256 tokenId) public returns (bool)
    {
        bytes32 key = keccak256(abi.encodePacked(a,b,c, input));
